...
function Notifications() {
  ...
  return (
    <div className="max-w-[800px] mx-auto my-0">
      ...
      <div className="rounded-2xl bg-[#faf9f6] p-2 my-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div
            className={cn(
              "flex items-center justify-between gap-1 cursor-pointer rounded-2xl p-2 px-4",
              active === "all" ? "bg-white" : ""
            )}
            onClick={() => setActive("all")}
          >
            <h1 className="text-gray-500 text-sm font-karla">All</h1>
          </div>
          <div
            className={cn(
              "flex items-center justify-between gap-1 cursor-pointer rounded-2xl p-2 px-4",
              active === "messages" ? "bg-white" : ""
            )}
            onClick={() => setActive("messages")}
          >
            <h1 className="text-gray-500 text-sm font-karla">Messages</h1>
          </div>
        </div>
